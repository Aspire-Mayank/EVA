...
class CarEnv(object):
    ...
    def get_state(self, ax=None):
        ...
        crop_img = utilityImage.center_crop_img(img, self.pos.x, self.pos.y, self.crop_size*3)
        if ax is not None:
        ...
        crop_img = utilityImage.center_crop_img(r_img, int(r_img_x/2), int(r_img_y/2), self.crop_size)
        if ax is not None:
